...
def discrete_wavelet_transform_1d(signal):
    ...
    ##低频和高频系数
    cA = np.convolve(signal, low_pass, mode='valid')[::2]
    cD = np.convolve(signal, high_pass, mode='valid')[::2]
...
def inverse_wavelet_transform_1d(cA, cD):
    ...
    # 逆小波变换
    reconstructed_signal = np.convolve(upsampled_cA, low_pass, mode='valid') + np.convolve(upsampled_cD, high_pass, mode='valid')
...
def inverse_wavelet_transform(cA, cV, cH, cD):
    ...
    # 对每一行进行逆小波变换
    for i in range(n_rows * 2-7):
        reconstructed_image[i, :] = inverse_wavelet_transform_1d(rows_transformed_0[i, :], rows_transformed_1[i, :])
    
    reconstructed_image = cv2.resize(reconstructed_image, (n_cols * 2, n_rows * 2), interpolation=cv2.INTER_LINEAR)
...
def process(image, level=1):
    ...
    reconstructed_cA = cv2.resize(reconstructed_cA, (cV.shape[0], cV.shape[1]), interpolation=cv2.INTER_LINEAR)
