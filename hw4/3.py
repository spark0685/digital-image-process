...
def inverse_wavelet_transform_1d(cA, cD):
    ...
    # 逆小波变换
    reconstructed_signal = np.convolve(upsampled_cA, low_pass, mode='full') + np.convolve(upsampled_cD, high_pass, mode='full')
...
def inverse_wavelet_transform(cA, cV, cH, cD):
    ...
    # 初始化重建的行
    rows_transformed_0 = np.zeros((n_rows * 2+7, n_cols))
    rows_transformed_1 = np.zeros((n_rows * 2+7, n_cols))
    ...
    # 初始化最终重建的图像
    reconstructed_image = np.zeros((n_rows * 2+7, n_cols * 2+7))
    
    # 对每一行进行逆小波变换
    for i in range(n_rows * 2):
        reconstructed_image[i, :] = inverse_wavelet_transform_1d(rows_transformed_0[i, :], rows_transformed_1[i, :])
    
    reconstructed_image = cv2.resize(reconstructed_image, (n_cols * 2, n_rows * 2), interpolation=cv2.INTER_LANCZOS4)
...
def process(image):
    ...
    # 逆小波变换
    reconstructed_cA_1 = inverse_wavelet_transform(cA2, cV2, cH2, cD2)

    cV2 = np.zeros_like(cV2)
    cV = np.zeros_like(cV)

    reconstructed_cA_2 = inverse_wavelet_transform(cA2, cV2, cH2, cD2)

    #对reconstructed_cA进行resize
    reconstructed_cA_1 = cv2.resize(reconstructed_cA_1, (cA.shape[0], cA.shape[1]), interpolation=cv2.INTER_LINEAR)
    
    reconstructed_cA_2 = cv2.resize(reconstructed_cA_2, (cA.shape[0], cA.shape[1]), interpolation=cv2.INTER_LINEAR)
    
    # 进行逆小波变换
    reconstructed_image_1 = inverse_wavelet_transform(reconstructed_cA_1, cV, cH, cD)

    reconstructed_image_2 = inverse_wavelet_transform(reconstructed_cA_2, cV, cH, cD)

    #对reconstructed_image进行归一化
    reconstructed_image_1 = cv2.normalize(reconstructed_image_1, None, 0, 255, cv2.NORM_MINMAX)
    reconstructed_image_2 = cv2.normalize(reconstructed_image_2, None, 0, 255, cv2.NORM_MINMAX)

    return reconstructed_image_1, reconstructed_image_2
...
# 处理图像以获取最终结果
result1, result2 = process(image)


# 保存结果
cv2.imwrite("result\\3-1.jpg", result1)

cv2.imwrite("result\\3-2.jpg", result2)

cv2.imwrite("result\\3-3.jpg", image)
